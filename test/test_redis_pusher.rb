...
class TestRedisPusher < Minitest::Test
  ...
  def test_single_push
    ...
    @redis_pusher.single_push(test_string, redis_list_name)
    assert_equal test_string, Redis.new.lrange(redis_list_name, 0, -1)
  end

  def test_pipelined_push
    test_array = ["test1", "test2", "test3"]
    redis_list_name = "PIPE_TEST"
    @redis_pusher.pipeline_push(redis_list_name, test_array)
